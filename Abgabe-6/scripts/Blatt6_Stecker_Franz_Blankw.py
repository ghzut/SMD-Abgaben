...
P_Temp_kalt = 6/14
P_Ausb_sonnig = 3/14
P_W = P_Wind_stark * P_Feucht_hoch * P_Temp_kalt * P_Ausb_sonnig
P_F_W = P_Wind_stark_F * P_Feucht_hoch_F * P_Temp_kalt_F * P_Ausb_sonnig_F * P_F/P_W
print(P_F_W)
#c)
...
#Nr.17)
temperature, weather, humidity, wind, soccer = np.genfromtxt('scripts/data.txt', unpack=True)
makeTable([temperature, weather, humidity, wind, soccer], r'{'+r'$Temperature/\si{\celsius}$'+r'} & {'+r'$Weather$'+r'} & {'+r'$Humidity/\si{\percent}$'+r'} & {'+r'$Wind$'+r'} & {'+r'$Soccer$'+r'}','tabData',['S[table-format=2.1]', 'S[table-format=1.0]', 'S[table-format=2.0]', 'S[table-format=1.0]', 'S[table-format=1.0]'],["%2.1f", "%1.0f", "%2.0f", "%1.0f", "%1.0f"])
...
plt.ylabel(r'$ IG$')
plt.legend(loc='best')
plt.savefig('build/humidity.pdf')
plt.clf()
