...
#f             durch 0 teilen und ähnliches macht diesen Aufgabenteil nicht möglich meiner meinung nach...
...
print("Für P_0_10000: \lambda_{S/B_{max}}")
print(lambda_SB_max)
...
plt.legend(loc='best')
plt.xlabel(r'$\lambda$')
plt.savefig('build/Aufgabe_2_gd.pdf')
plt.clf()
...
plt.xlabel(r'$\lambda$')
plt.legend(loc='best')
plt.savefig('build/Aufgabe_2_ge.pdf')
plt.clf()
...
plt.plot(lambda_cut,y)
lambda_SB_max=lambda_cut[np.argmax(y)]
print("Für P_0_1000: \lambda_{S/B_{max}}")
print(lambda_SB_max)
#plt.vlines(lambda_SB_max,0,np.max(y))
plt.xlabel(r'$\lambda$')
plt.ylabel(r'$\frac{Signal}{Untergrund}$')
#plt.legend(loc='best')
plt.savefig('build/Aufgabe_2_gf.pdf')
plt.clf()